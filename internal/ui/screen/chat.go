...
func (s *ChatScreen) newMessagePager(focused *slack.Message, showHistory bool) *component.Pager {
	var content string
	if showHistory {
		content = renderMessageWithHistory(s.formatter, focused, s.width)
	} else {
	...
	p := component.NewPager(content, s.width, msgHeight)
	return &p
}
